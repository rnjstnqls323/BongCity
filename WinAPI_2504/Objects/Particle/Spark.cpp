...
void Spark::Update()
{
    ...
    buffer->GetValues()[0] += DELTA;

    if (buffer->GetValues()[0] > buffer->GetValues()[1])
        Stop();
}
...
void Spark::GUIRender()
{
    ...
    ImGui::ColorEdit4("StartColor", startColorbuffer->GetValues());
    ImGui::ColorEdit4("EndColor", endColorbuffer->GetValues());
}

void Spark::Play(Vector3 position)
{
    buffer->GetValues()[0] = 0.0f;
